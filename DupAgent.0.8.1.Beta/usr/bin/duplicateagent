...
from PySide6.QtCore import Qt, Signal, Slot, QThread, QLocale, QSize, QFileInfo # <<< QFileInfo Ekledik-Bunları sahte çöp kutusu için kullanacağız.
...
class FakeTrashManager:
    """Sahte Çöp Kutusu dizinlerini ve metadata dosyasını yönetir."""

    def __init__(self):
        self.base_dir = os.path.join(os.path.expanduser('~'), '.duplicateagent')
        self.trash_dir = os.path.join(self.base_dir, 'FakeTrash')
        self.metadata_path = os.path.join(self.base_dir, 'trashdata.json')
        self._setup_dirs()

    def _setup_dirs(self):
        """Gerekli dizinleri ve metadata dosyasını oluşturur."""
        try:
            ...
            print(f"HATA: Fake Trash dizinleri oluşturulamadı: {e}")

    def _load_metadata(self):
        """Metadata dosyasını okur ve JSON listesi döndürür."""
        try:
    ...
    def _save_metadata(self, data):
        """Metadata listesini dosyaya yazar."""
        try:
            with open(self.metadata_path, 'w', encoding='utf-8') as f:
    ...
    def move_to_trash(self, filepath, file_size_bytes):
        """Dosyayı FakeTrash'a taşır ve metadata kaydını oluşturur."""
        ...
        # Ek karmaşıklık için milisaniyeye dayalı bir postfix ekleyelim
        postfix = int(datetime.now().timestamp() * 1000) 
        trash_filename = f"{name}_{postfix}{ext}"

        while os.path.exists(os.path.join(self.trash_dir, trash_filename)):
            counter += 1
            trash_filename = f"{name}_{postfix}_{counter}{ext}"

        target_path = os.path.join(self.trash_dir, trash_filename)
    ...
    def get_trash_files(self):
        """Tüm çöp kutusu dosyalarının metadata listesini döndürür."""
        return self._load_metadata()

    def restore_file(self, trash_filename, original_path):
        """Dosyayı FakeTrash'tan orijinal konumuna geri yükler."""
        trash_file_path = os.path.join(self.trash_dir, trash_filename)
        
        if not os.path.exists(trash_file_path):
            return False # Dosya zaten silinmiş veya mevcut değil

        try:
            # Orijinal klasör yolunu kontrol et ve oluştur (Yoksa tabi)
    ...
    def purge_file(self, trash_filename, original_path):
        ...
        try:
            ...
            # Metadata kaydını sil
            # İki tane olmasının nedeni kalıcı olarak sil fonksiyonunun var olmasıdır. 
            metadata = self._load_metadata()
            metadata = [item for item in metadata if not (item["trash_filename"] == trash_filename and item["original_path"] == original_path)]
            self._save_metadata(metadata)
...
# "Allah dünya üzerinde yarattığı bu kadar nimetleri bu kadar güzellikleri insanlar istifade etsin varlık içinde yaşasın diye yaratmıştır ve azamî derecede faydalanabilmek için de bütün yaratıklardan esirgediği zekâyı akıllı insanlara vermiştir." M. Kemal ATATÜRK
    ...
    def _setup_ui(self):
        ...
        new_main_layout.addWidget(settings_panel)
    ...
    def _show_about(self):
        ...
        <ul>
            <li><b>{get_text("about_version")}:</b> 0.8.1 (beta)</li>
    ...
    def update_trash_tab(self):
        """Fake Trash sekmesindeki tabloyu günceller."""
        trash_data = self.trash_manager.get_trash_files()
        ...
        for item in trash_data:
            ...
            # <<< YENİ: İKON EKLEME >>>
            trash_file_path = os.path.join(self.trash_manager.trash_dir, trash_filename)
            ...
            # 2. Orijinal Yolu (UserRole ile orijinal yolu da saklayalım)
            original_path = item.get("original_path", "")
            original_path_item = QTableWidgetItem(original_path)
            # İleride kullanmak için trash_filename'i de Data olarak sakla
            original_path_item.setData(Qt.UserRole, item.get("trash_filename", ""))
    ...
    def _get_selected_trash_items(self):
        """Çöp tablosunda seçilen dosyaların listesini döndürür (trash_filename, original_path)."""
        selected_items = []
        for row in range(self.trash_table.rowCount()):
            check_item = self.trash_table.item(row, 0)
            if check_item and check_item.checkState() == Qt.CheckState.Checked:
                trash_filename = self.trash_table.item(row, 1).text() 
                original_path = self.trash_table.item(row, 2).text() 
                selected_items.append({"trash_filename": trash_filename, "original_path": original_path})
    ...
    def _restore_selected_files(self):
        ...
        for file_data in selected_files:
            if self.trash_manager.restore_file(file_data["trash_filename"], file_data["original_path"]):
                restored_count += 1
            else:
                error_count += 1
    ...
    def _purge_selected_files(self):
        ...
        for file_data in selected_files:
            if self.trash_manager.purge_file(file_data["trash_filename"], file_data["original_path"]):
                purged_count += 1
            else:
                error_count += 1
